<!DOCTYPE html>
<html>
    
    <head>
        <title>Ask ALL</title>
        <link rel="stylesheet" type="text/css" href="css/style.css"/>
    </head>

    <body>
        <div id="container">

            <main id="header-auth">
                <p id="logo">Ask ALL</p>
            </main>
            
            <main id="profile-auth">
                <div id="pSet">
                    <img id="uPhoto" alt="" src="images/KJY.jpg"/>
                    <p id="uName">JinnieK</p>
                    
                    <div class="graph_wrap">
                        
                        <div class="graph">
                          <ul class="y-axis">
                            <li><span>35</span></li>
                            <li><span>30</span></li>
                            <li><span>25</span></li>
                            <li><span>20</span></li>
                            <li><span>15</span></li>
                            <li><span>10</span></li>
                            <li><span>5</span></li>
                            <li><span>0</span></li>
                          </ul>
                      
                          <ul class="x-axis">
                            <li><span>Hour</span></li>
                            <li><span>Day</span></li>
                            <li><span>Week</span></li>
                          </ul>
                      
                          <ul class="bar">
                            <li style="height:10%;"><span></span></li>
                            <li style="height:15%;"><span></span></li>
                            <li style="height:30%;"><span></span></li>
                          </ul>
                        </div>
                      </div>

                    <button type="button" onclick="location.href='login.html'">Logout</button>
                </div>
            </main>
            
            <main id="main-left"></main>
            
            <main id="main-center">
                
                <div class="qaSet">
                    <div class="qa">
                        <div class="qnb">
                            <div class="question">
                                <p class="q">Q. What is Senioritis?</p>
                                <p class="p_td">
                                    <span class="t">4:01 PM </span>
                                    <span class="d">Jan 29, 2023</span>
                                </p>
                                <span class="p_u">user1</span>
                                <img class="p_p" alt="" src="images/unknown.png"/>
                            </div>
                            <button type="button" onclick="location.href='a.html'">Go</button>
                        </div>
                        <div class="answers">
                            <div class="answer">
                                <p class="a">A. We are for technical problems, not vocabulary.</p>
                                <p class="p_td">
                                    <span class="t">4:11 PM </span>
                                    <span class="d">Jan 29, 2023</span>
                                </p>
                                <span class="p_u">user20</span>
                                <img class="p_p" alt="" src="images/unknown.png"/>
                            </div>
                            <div class="answer">
                                <p class="a">A. a supposed affliction of students in their final year of high school or college, characterized by a decline in motivation or performance.</p>
                                <p class="p_td">
                                    <span class="t">4:02 PM </span>
                                    <span class="d">Jan 29, 2023</span>
                                </p>
                                <span class="p_u">user35</span>
                                <img class="p_p" alt="" src="images/unknown.png"/>
                            </div> 
                        </div>
                    </div>
                    
                </div>

                <div class="qaSet">
                    <div class="qa">
                        <div class="qnb">
                            <div class="question">
                                <p class="q">Q. Do you know who I am? I am a one of exchange students from North Korea.</p>
                                <p class="p_td">
                                    <span class="t">2:51 PM </span>
                                    <span class="d">Jan 29, 2023</span>
                                </p>
                                <span class="p_u">user6</span>
                                <img class="p_p" alt="" src="images/unknown.png"/>
                            </div>
                            <button type="button" onclick="location.href='a.html'">Go</button>
                        </div>
                        <div class="answers">
                            <div class="answer">
                                <p class="a">A. I am not a detective. How can I know you?</p>
                                <p class="p_td">
                                    <span class="t">2:56 PM </span>
                                    <span class="d">Jan 29, 2023</span>
                                </p>
                                <span class="p_u">user11</span>
                                <img class="p_p" alt="" src="images/unknown.png"/> 
                            </div>
                        </div>
                    </div>
                </div>
                
            </main>
            
            <main id="main-right"></main>
            
            <main id="footer-auth">
                <p id="copyrights">(C) 2023. JinYi Kim all rights reserved.</p>
            </main> 
        </div>
    </body>

</html>